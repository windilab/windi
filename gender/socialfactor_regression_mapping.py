...
# データ読み込み
df=pd.read_csv(r"C:\Users\piinb\Documents\DR\WIND\estat_rawdata.csv", encoding='Shift_JIS')
...
norm = plt.Normalize(vmin=df_result.male_female.min(), vmax=df_result.male_female.max())
fcol = lambda x: '#' + bytes(cmap(norm(x), bytes=True)[:3]).hex()

plt.rcParams['figure.figsize'] = 10, 10
plt.colorbar(plt.cm.ScalarMappable(norm, cmap))
plt.title('非労働力人口　男性回帰係数 - 女性回帰係数 のz値')
plt.imshow(picture(df_result.male_female.apply(fcol)))
